...
pub(super) fn process(
    ...
    parser.append_block_to(*id, MarkdownNode::Emoji(line.slice(0, end).to_string()), (start_location, end_location));
    line.skip(end + 1);
    true
}
...
mod tests{
    ...
    #[test]
    fn it_works(){
        let ast = Parser::new("@octocat :+1: This PR looks great - it's ready to merge! :狗头:").parse();
        println!("{ast:?}");
    }
}