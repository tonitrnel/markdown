...
fn semantic_digest_observes_html_changes() {
    let first = semantic_digest(&Parser::new("<div>first</div>").parse());
    let second = semantic_digest(&Parser::new("<div>second</div>").parse());
