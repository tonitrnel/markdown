...
pub(crate) fn node_payloads_to_json(doc: &Document) -> serde_json::Result<String> {
...
fn write_payload_subtree(
    ...
    *first = false;
    ...
    if let Some(block_id) = &node.id {
        serde_json::to_writer(&mut *out, block_id.as_str())?;
    } else {
...
fn write_content(doc: &Document, body: &MarkdownNode, out: &mut Vec<u8>) -> serde_json::Result<()> {
    match body {
        // TextRef 载荷：物化后写出（Text 直接借 resolved 字符串，零克隆）
        MarkdownNode::Text(t) => {
            out.extend_from_slice(b",\"content\":");
            serde_json::to_writer(&mut *out, doc.text(t))?;
        }
        MarkdownNode::Link(link) => {
            ...
            out.extend_from_slice(b",\"content\":");
            serde_json::to_writer(&mut *out, &*link)?;
        }
        MarkdownNode::Image(image) => {
            ...
            out.extend_from_slice(b",\"content\":");
            serde_json::to_writer(&mut *out, &*image)?;
        }
