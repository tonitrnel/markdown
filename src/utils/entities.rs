...
// ```js
// let intl = Intl.Collator("en");
// const sets = new Set();
// document.body.innerText = Object.entries(JSON.parse(temp1.textContent)).map(([key, value]) => [key.toLowerCase().replace(/[&;]/g, ''), value["codepoints"].map(it => `\\u{${it.toString(16).padStart(4, '0')}}`).join("")]).filter(([key,_])=>{
//     if (sets.has(key)) return false;
//     else {
//         sets.add(key);
//         return true
//     }
// }).sort((a, b) => intl.compare(a[0], b[0])).map(([key, value]) => `(b"${key}", "${value}")`).join(",\n");
// ````

static ENTITIES: [(&[u8], &str); 2125] = [
    (b"AElig", "\u{00C6}"),
    (b"AMP", "\u{0026}"),
    (b"Aacute", "\u{00C1}"),
    (b"Abreve", "\u{0102}"),
...
pub(crate) fn lookup_entity(bytes: &[u8]) -> Option<&'static str> {
    ENTITIES
...
pub(crate) fn unescape_string(str: impl AsRef<str>) -> String {
    let str = str.as_ref();
    if str.contains('&') {
        let mut state = EscapeState::Initial;
        ...
        for char in str.chars() {
            match (&state, char) {
                (EscapeState::Initial, '&') => {
                    buf.push(char);
                    state = EscapeState::Entity
                }
                (EscapeState::Initial, ch) => unescaped.push(ch),
                (EscapeState::Entity, '#') => {
                    buf.push(char);
                    state = EscapeState::Numeric
                }
                (EscapeState::Entity, ch) if ch.is_ascii_alphabetic() || ch.is_ascii_digit() => {
                    buf.push(ch);
                    state = EscapeState::Named;
                }
                (EscapeState::Named, ';') => {
                    state = EscapeState::Initial;
                    if let Some(str) = lookup_entity(buf[1..].as_bytes()) {
                        unescaped.push_str(str);
                        buf.clear();
                    } else {
                        unescaped.extend(buf.drain(..));
                        unescaped.push(';');
                    }
                }
                (EscapeState::Named, ch) => buf.push(ch),
                (EscapeState::Numeric, ch) if ch.is_ascii_digit() => {
                    buf.push(ch);
                    state = EscapeState::Dec;
                }
                (EscapeState::Numeric, ch) if ch == 'x' || ch == 'X' => {
                    buf.push(char);
                    state = EscapeState::Hex;
                }
                (EscapeState::Dec, ';') => {
                    state = EscapeState::Initial;
                    if let Ok(Some(mut ch)) = buf[2..].parse::<u32>().map(char::from_u32) {
                ...
                (EscapeState::Hex, ch) if ch.is_ascii_hexdigit() => buf.push(ch),
                (_, ch) => {
                    if !buf.is_empty() {
                        unescaped.extend(buf.drain(..));
                    }
        ...
        if !matches!(state, EscapeState::Initial) {
            unescaped.extend(buf.drain(..));
        }
        unescaped
    } else {
...
mod tests {
    ...
    #[test]
    fn it_works() {
        assert_eq!(lookup_entity(b"amp"), Some("&"));
        assert_eq!(lookup_entity(b"lt"), Some("<"));
        assert_eq!(lookup_entity(b"gt"), Some(">"));
        assert_eq!(lookup_entity(b"quot"), Some("\""));
    }
    #[test]
    fn test_unescaped_string() {
        ...
        assert_eq!(unescape_string("&copy"), "&copy");
